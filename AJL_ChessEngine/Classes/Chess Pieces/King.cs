...
namespace AJL_ChessEngine
{
    ...
    {
        public King(Color givenTeam) : base(givenTeam)
        {
            id = givenTeam == Color.white ? (byte)FigID.wKing : (byte)FigID.bKing;
            worth = 100;
        }
        public King(State givenState, pos givenPosition, Color givenTeam) : base(givenState, givenPosition, givenTeam)
        {
            id = givenTeam == Color.white ? (byte)FigID.wKing : (byte)FigID.bKing;
            worth = 100;
        }
